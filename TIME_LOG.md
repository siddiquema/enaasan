...
**0h 0m** — tracking starts from the next session's start timestamp. This session's true duration is not retroactively known (no start timestamp was logged), so it is excluded from the total to avoid a misleading number.
